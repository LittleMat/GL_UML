...
int menu(int argc, char **argv)
{
	...
	while (stoi(lecture) != 4)
	{
		...
		case 1:
		{
			...
			function<float(Capteur &)> fiabilite = bind(Service::fiabilite, _1, cref(territoire));
			function<bool(Mesure &)> predicateMesure = bind(Service::filtrageMesure, _1, date1finale, date2finale);
		...
		case 3:
		{
			...
			function<bool(Mesure&)> predicateMesure = bind(Service::filtrageMesure, _1, tm(), tm());
